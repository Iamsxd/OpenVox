...
const normalizedBase =
  expectedBase === "/" ? "/" : `/${expectedBase.replace(/^\/+|\/+$/g, "")}/`;
const publicUrl = (
  process.env.OPENVOX_PUBLIC_URL || "https://iamsxd.github.io/OpenVox"
).replace(/\/+$/, "");
...
if (html.includes("%BASE_URL%"))
  throw new Error("Unresolved Vite base token found in index.html.");
if (html.includes("/src/main.tsx"))
  throw new Error("Raw Vite source entry found in production index.html.");
if (!html.includes(`href=\"${normalizedBase}manifest.webmanifest\"`))
  throw new Error(
    "Production manifest URL does not use the configured Pages base.",
  );
if (!html.includes(`href=\"${normalizedBase}openvox-icon.svg\"`))
  throw new Error(
    "Production icon URL does not use the configured Pages base.",
  );
...
if (!html.includes('data-theme="system"'))
  throw new Error(
    "System theme bootstrap is missing from the production HTML.",
  );
if (
  !html.includes(
    "primary === 'zh' ? 'zh' : primary === 'uk' ? 'uk' : primary === 'de' ? 'de' : 'en'",
  )
)
  throw new Error(
    "Automatic interface-language bootstrap is missing from the production HTML.",
  );

const assetDir = resolve(dist, "assets");
const assetFiles = await readdir(assetDir);
const appBundleName = assetFiles.find(
  (name) => name.startsWith("index-") && name.endsWith(".js"),
);
if (!appBundleName)
  throw new Error("Application JavaScript bundle is missing.");
...
if (/\/api\/(?:auth|sync)|AuthProvider|AccountPage/.test(appBundle))
  throw new Error(
    "Application bundle must not contain account, authentication, or synchronization features.",
  );
if (
  !appBundle.includes(normalizedBase) ||
  !appBundle.includes("pro-lab/dsp-library.js")
)
  throw new Error(
    "Pro Audio Lab DSP path does not use the configured Pages base.",
  );
if (!appBundle.includes("pro-lab/pro-audio-lab.js"))
  throw new Error(
    "Pro Audio Lab runtime path is missing from the application bundle.",
  );
...
if (
  ...
  throw new Error(
    `PWA base path is incorrect (${manifest.start_url}, ${manifest.scope}).`,
  );
}
...
if (
  !serviceWorker.includes("/wasm/") ||
  !serviceWorker.includes("/worklets/") ||
  !serviceWorker.includes("/pro-lab/")
)
  throw new Error("Service worker runtime revalidation rules are incomplete.");
if (serviceWorker.includes("workbox"))
  throw new Error(
    "Unexpected Workbox runtime detected in the custom service worker.",
  );
...
if (!scoreHtml.includes(`${publicUrl}/score/`))
  throw new Error("Score route canonical URL is incorrect.");

const sitemap = await readFile(resolve(dist, "sitemap.xml"), "utf8");
if (!sitemap.includes(`${publicUrl}/studio/`))
  throw new Error("Sitemap does not contain the Pages public URL.");

const unexpectedWorkboxFiles = [];
await walk(dist, async (full, name) => {
  if (/^workbox-.*\.js$/i.test(name))
    unexpectedWorkboxFiles.push(relative(dist, full));
});
if (unexpectedWorkboxFiles.length)
  throw new Error(
    `Unexpected Workbox files found: ${unexpectedWorkboxFiles.join(", ")}`,
  );
...
if (sourceMaps.length)
  throw new Error(
    `Source maps should not be deployed: ${sourceMaps.join(", ")}`,
  );

const wasmSize = (await stat(resolve(dist, "wasm/openvox_dsp.wasm"))).size;
if (wasmSize < 500)
  throw new Error("WASM DSP module appears unexpectedly small.");
