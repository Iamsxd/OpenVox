...
const pages = {
  ...
  privacy: [
    "Privacy",
    "Learn how OpenVox Studio processes microphone audio and project data locally on your device.",
  ],
};
...
function withSeo(html, route, title, description) {
  const pageTitle = route
    ? `${title} — OpenVox Studio`
    : "OpenVox Studio — Your voice. Understood.";
  ...
  output = output.replace(
    /"url":\s*"https:\/\/[^\"]+\/OpenVox(?:-Studio)?\/"/,
    `"url": "${publicUrl}/"`,
  );
  output = output.replace(
    /"url":\s*"https:\/\/authorche\.top\/openvox\/"/,
    `"url": "${publicUrl}/"`,
  );
  return output;
}
...
await writeFile(
  templatePath,
  withSeo(template, "", "OpenVox Studio", homeDescription),
);
...
for (const [route, [title, description]] of Object.entries(pages)) {
  ...
  await writeFile(
    ...
    withSeo(template, route, title, description),
  );
}
...
const sitemap = [
  ...
  ...routes.map((route, index) => {
    ...
    const priority =
      index === 0
        ? "1.0"
        : ["studio", "academy", "practice", "instruments"].includes(route)
          ? "0.9"
          : "0.7";
...
const manifest = {
  ...
  icons: [
    {
      src: `${base}icons/openvox-192.png`,
      sizes: "192x192",
      type: "image/png",
      purpose: "any",
    },
    {
      src: `${base}icons/openvox-512.png`,
      sizes: "512x512",
      type: "image/png",
      purpose: "any maskable",
    },
    {
      src: `${base}openvox-icon.svg`,
      sizes: "any",
      type: "image/svg+xml",
      purpose: "any maskable",
    },
...
for (const file of allFiles) {
  ...
  const extension = rel.includes(".")
    ? rel.slice(rel.lastIndexOf(".")).toLowerCase()
    : "";
