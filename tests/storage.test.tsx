...
describe("local IndexedDB storage", () => {
  it("persists settings and sorts projects by last update", async () => {
    ...
    expect((await listProjects()).map((item) => item.id)).toEqual([
      "newer",
      "older",
    ]);
  ...
  it("persists training sessions and removes them with a project", async () => {
    ...
    expect(
      (await listTrainingSessions(current.id)).map((item) => item.id),
    ).toEqual(["session-1"]);
  ...
  it("persists and deletes practice goals locally", async () => {
    ...
    expect((await listPracticeGoals()).map((item) => item.id)).toEqual([
      "local-goal",
    ]);

    await deletePracticeGoal(localGoal.id);
