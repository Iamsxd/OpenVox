...
describe("OpenVox application shell", () => {
  ...
  it("switches the complete interface to Simplified Chinese", async () => {
    ...
    expect(
      screen.getByRole("heading", { level: 2, name: "界面" }),
    ).toBeTruthy();
...
describe("Tool-first home and mobile-ready score UI", () => {
  it("opens the primary local workflows without creator or support promotions", async () => {
    ...
    expect(
      container.querySelector('.home-primary-grid a[href="/studio"]'),
    ).toBeTruthy();
    expect(
      container.querySelector('.home-primary-grid a[href="/practice"]'),
    ).toBeTruthy();
    expect(
      container.querySelector('.home-primary-grid a[href="/academy"]'),
    ).toBeTruthy();
    expect(
      container.querySelector('.home-primary-grid a[href="/analyze"]'),
    ).toBeTruthy();
    expect(screen.queryByText("Created by AuthorChe")).toBeNull();
    expect(
      screen.queryByRole("button", { name: "Support project" }),
    ).toBeNull();
  });
