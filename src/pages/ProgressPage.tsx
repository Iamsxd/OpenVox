...
export function ProgressPage() {
  ...
  const [goalTitle, setGoalTitle] = useState(() =>
    proText(language, "progress.weeklyDefault"),
  );
  ...
  useEffect(() => {
    void reload();
    window.addEventListener(TRAINING_CHANGED_EVENT, reload);
    return () => window.removeEventListener(TRAINING_CHANGED_EVENT, reload);
  }, []);
  ...
  const stats = useMemo(() => {
    ...
    const accuracy = scored.length
      ? scored.reduce((sum, s) => sum + (s.accuracy || 0), 0) / scored.length
      : null;
  ...
  const categoryStats = useMemo(() => {
    const map = new Map<
      string,
      { seconds: number; count: number; score: number; scored: number }
    >();
    sessions.forEach((session) => {
      const current = map.get(session.category) || {
        seconds: 0,
        count: 0,
        score: 0,
        scored: 0,
  ...
  return (
    <div className="page">
      ...
      <section className="practice-local-banner">
        <Icon name="shield" />
        <span>{x("progress.allLocal")}</span>
      </section>
              ...
              {sessions.slice(0, 100).map((session) => (
                <div className="history-row" key={session.id}>
                  ...
                  <span>
                    ...
                    <small>
                      {trainingCategoryName(language, session.category)} ·{" "}
                      {session.difficulty}
                    </small>
                  </span>
                  ...
                  <button
                    ...
                    onClick={() =>
                      void deleteTrainingSession(session.id).then(reload)
                    }
          ...
          <div className="field-row">
            ...
            <div className="field">
              ...
              <select
                ...
                onChange={(e) =>
                  setGoalCategory(
                    e.target.value as TrainingCategory | "general",
                  )
                }
              >
          ...
          <div className="goal-list">
            {goals.map((goal) => {
              const relevant = stats.thisWeek.filter(
                (s) =>
                  goal.category === "general" || s.category === goal.category,
              );
              const minutes =
                relevant.reduce((sum, s) => sum + s.durationSeconds, 0) / 60;
              const percent = Math.min(
                100,
                (minutes / goal.targetMinutesPerWeek) * 100,
              );
              return (
                <article key={goal.id} className="goal-item">
                  <div>
                    ...
                    <small>
                      {Math.round(minutes)} / {goal.targetMinutesPerWeek} min ·{" "}
                      {goal.category}
                    </small>
                  </div>
                  ...
                  <button
                    ...
                    onClick={() =>
                      void deletePracticeGoal(goal.id).then(reload)
                    }
                  >
                    {x("progress.remove")}
                  </button>
                ...
                return (
                  <div className="category-bar" key={category}>
                    ...
                    <div>
                      <i
                        style={{
                          width: `${Math.max(2, (data.seconds / max) * 100)}%`,
                        }}
                      />
                    </div>
